...
public class PageBase {
	...
	public String tool = null, webBrowser = null, chromeDriverPath = null, fireFoxDriverPath = null,
			...
			ParentWinhadleMob = null, ParentWinhadle = null, mobileCloud = null, bundleId = null;
		...
		if (locality.equalsIgnoreCase("Hub")) {
			if (browser.equalsIgnoreCase("chrome")) {
				...
				if (bmpValue.equalsIgnoreCase("Yes")) {
					...
					capabilities.setCapability(CapabilityType.PROXY, seleniumProxy);
					remoteDriver = new FirefoxDriver(capabilities);
	...
	public String takeScreenShot() {
		...
		File screen = null;
		try {
			String imgQuality = TestBase.getPropertyValue("imageQuality");
			//System.out.println(imgQuality);
			switch (toolName) {
			case "Appium":
				screen = (File) ((TakesScreenshot) appiumDriver).getScreenshotAs(OutputType.FILE);
				break;
			case "Selenium":
				screen = ((TakesScreenshot) remoteDriver).getScreenshotAs(OutputType.FILE);

				if (imgQuality.equalsIgnoreCase("low")) {

					String filePath = "ReportGenerator/" + ExtentUtility.reportFolder + "/Screenshots/image" + s
							+ ".png";
					String newfilePath = "ReportGenerator/" + ExtentUtility.reportFolder + "/Screenshots/img" + s
							+ ".png";
					FileUtils.copyFile(screen, new File(filePath));
					ScreenShotUtils.reduceScreenShotQuality(new File(filePath), new File(newfilePath).getAbsolutePath(),
							"", "png");
					File file = new File(filePath);
					if (file.delete()) {
					} else
						System.out.println(filePath + " does not exist");
				} else if (imgQuality.equalsIgnoreCase("high")) {

					String newfilePath1 = "ReportGenerator/" + ExtentUtility.reportFolder + "/Screenshots/img" + s
							+ ".png";
					FileUtils.copyFile(screen, new File(newfilePath1));
				}
				break;
			}
		...
		return ("Screenshots/img" + s + ".png");
	}

	private void compress(String filePath, String filePath2) throws IOException {
	...
	public void enterUrl(String url) throws Exception {
		try {
			 ...
			 * "Url "+url+"has been launched sucessfully",
			 * ExtentUtility.getTest().addScreenCapture(takeScreenShot()));
			 */
		} catch (Exception exc) {
		...
		try {
			...
			for (int i = 0; i < 3; i++) {
				...
				js.executeScript("arguments[0].setAttribute('style', arguments[1]);", e,
						"Yellow: green; border:4px solid green;");
				js.executeScript("arguments[0].setAttribute('style', arguments[1]);", e, "");
			}
	...
	public String getText(WebElement e, String elementName) throws Exception {
		...
		ExtentUtility.getTest().log(LogStatus.PASS, "Get the Text for" + elementName + " successful",
				ExtentUtility.getTest().addScreenCapture(takeScreenShot()));
		return text;
	...
	public void selectOPtionByVisibleText(WebElement e, String text) throws InterruptedException, IOException {
		try {
			...
			ExtentUtility.getTest().log(LogStatus.PASS, "Selected Option by Visible Text " + e,
					ExtentUtility.getTest().addScreenCapture(takeScreenShot()));
		}
	...
	public boolean elementIsDisplayed(WebElement e) throws Exception {
		try {
			...
			ExtentUtility.getTest().log(LogStatus.PASS, " Element " + e + "is displayed ",
					ExtentUtility.getTest().addScreenCapture(takeScreenShot()));
			return true;
